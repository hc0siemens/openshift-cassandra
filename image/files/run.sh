...
mkdir -p /cassandra_data/etc
mkdir -p /cassandra_data/hints
mkdir -p /cassandra_data/log
mkdir -p /cassandra_data/data
cp -rp /etc/cassandra/* /cassandra_data/etc/
export CASSANDRA_CONF="/cassandra_data/etc"

set -e
CASSANDRA_CONF_DIR=/cassandra_data/etc
...
if [[ $CASSANDRA_SEEDS == 'false' ]]; then
  sed -ri 's/- seeds:.*/- seeds: "'"$CASSANDRA_LISTEN_ADDRESS"'"/' $CASSANDRA_CFG
...
if [[ $CASSANDRA_OPEN_JMX == 'true' ]]; then
  ...
  sed -ri 's/ -Dcom\.sun\.management\.jmxremote\.password\.file=\/cassandra_data\/etc\/jmxremote\.password//' $CASSANDRA_CONF_DIR/cassandra-env.sh
fi

export CLASSPATH=/kubernetes-cassandra.jar
cassandra -R -f -Dcassandra.config=file:///cassandra_data/etc/cassandra.yaml 
